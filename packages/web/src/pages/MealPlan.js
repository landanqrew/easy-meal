...
export default function MealPlan() {
    ...
    const [pickerTypeFilter, setPickerTypeFilter] = useState(null);
    ...
    const fetchEntries = useCallback(async () => {
        try {
            const res = await fetch(`${API_URL}/api/meal-plans?weekStart=${formatDateParam(weekStart)}`, { credentials: 'include' });
    ...
    const getEntriesForSlot = (date, mealType) => {
    ...
    if (isPending || loading) {
        ...
        <div style={styles.content}>
          ...
          {isMobile ? (<div style={styles.mobileList}>
              {Array.from({ length: 7 }).map((_, i) => (<div key={i} style={styles.mobileDay}>
                  ...
                  <div style={styles.mobileMeals}>
                    {Array.from({ length: 3 }).map((_, j) => (<div key={j} style={styles.mealSlot}>
            ...
            </div>) : (<div style={styles.calendarGrid}>
              {Array.from({ length: 7 }).map((_, i) => (<div key={i} style={styles.dayColumn}>
    ...
    return (<div style={styles.container}>
      <div style={styles.content}>
        ...
        <div style={styles.mobileList}>
            {days.map((day, i) => {
                ...
                return (<div key={i} style={styles.mobileDay}>
                  <div style={{
                  ...
                  <div style={styles.mobileMeals}>
                    {MEAL_TYPES.map((mealType) => {
                        ...
                        return (<div key={mealType} style={styles.mealSlot}>
                          <div style={styles.mealSlotLabel}>
                            {mealType}
                          </div>
                          {slotEntries.map((entry) => (<div key={entry.id} style={styles.assignedRecipeRow}>
                              <Link to={`/recipes/${entry.recipe.id}`} style={styles.assignedRecipe}>
                                {entry.recipe.title}
                              </Link>
                              <button onClick={() => handleRemoveEntry(entry.id)} style={styles.removeButton}>
                                ×
                              </button>
                            </div>))}
                          <button onClick={() => setPickerOpen({
        ...
        <div style={styles.calendarGrid}>
            {days.map((day, i) => {
                const isToday = isSameDay(day, today);
                return (<div key={i} style={styles.dayColumn}>
                  <div style={{
                        ...styles.dayHeader,
      ...
      {/* Recipe picker modal */}
      {pickerOpen && (<div className="modal-overlay" onClick={(e) => {
          ...
          <div className="modal-card">
            ...
            <div style={styles.pickerTypeFilters}>
              {PICKER_TYPE_FILTERS.map((t) => (<button key={t.label} onClick={() => setPickerTypeFilter(pickerTypeFilter === t.value ? null : t.value)} className={`filter-chip${pickerTypeFilter === t.value ? ' active' : ''}`} style={{ fontSize: '0.6875rem', padding: '0.25rem 0.5rem' }}>
                  {t.label}
                </button>))}
            </div>
            <input type="text" placeholder="Search recipes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={styles.pickerSearch} autoFocus/>
            <div style={styles.pickerList}>
              {filteredRecipes.length === 0 ? (<p style={styles.pickerEmpty}>
                  {recipes.length === 0 ? 'No recipes yet. Create one first!' : 'No recipes match your search.'}
                </p>) : (filteredRecipes.map((recipe) => (<button key={recipe.id} onClick={() => handleAssignRecipe(recipe.id)} className="picker-recipe">
