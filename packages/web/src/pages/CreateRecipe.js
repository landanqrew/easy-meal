...
export default function CreateRecipe() {
    ...
    const [recipeType, setRecipeType] = useState('full_meal');
    // Generated recipe
    ...
    const addCustomItem = (value, existing, setCustom, setInput) => {
    ...
    return (<div style={styles.container}>
      <div style={styles.card}>
        ...
        {/* Step 1: Protein */}
        {step === 1 && (<div style={styles.stepContent}>
            <h2 style={styles.stepTitle}>What protein would you like?</h2>
            <div style={styles.selectionGrid}>
              {allProteins.map((p) => (<button key={p} onClick={() => setProtein(protein === p ? null : p)} className={`selection-card${protein === p ? ' selected' : ''}`}>
                  {p}
                </button>))}
            </div>
            <div className="custom-input-row">
              <input type="text" placeholder="Add other protein..." value={customProteinInput} onChange={(e) => setCustomProteinInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    addCustomItem(customProteinInput, allProteins, setCustomProteins, setCustomProteinInput);
                }
            }}/>
              <button onClick={() => addCustomItem(customProteinInput, allProteins, setCustomProteins, setCustomProteinInput)}>
                Add
              </button>
            </div>
        ...
        {/* Step 2: Vegetables */}
        {step === 2 && (<div style={styles.stepContent}>
            <h2 style={styles.stepTitle}>Which vegetables? (select any)</h2>
            <div style={styles.selectionGrid}>
              {allVegetables.map((v) => (<button key={v} onClick={() => setVegetables(vegetables.includes(v)
                    ? vegetables.filter((x) => x !== v)
                    : [...vegetables, v])} className={`selection-card${vegetables.includes(v) ? ' selected' : ''}`}>
                  {v}
                </button>))}
            </div>
            <div className="custom-input-row">
              <input type="text" placeholder="Add other vegetable..." value={customVegetableInput} onChange={(e) => setCustomVegetableInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    addCustomItem(customVegetableInput, allVegetables, setCustomVegetables, setCustomVegetableInput);
                }
            }}/>
              <button onClick={() => addCustomItem(customVegetableInput, allVegetables, setCustomVegetables, setCustomVegetableInput)}>
                Add
              </button>
            </div>
            {vegetables.length > 0 && (<p style={styles.selectionSummary}>Selected: {vegetables.join(', ')}</p>)}

            <h2 style={{ ...styles.stepTitle, marginTop: '1.5rem' }}>Any fruits? (select any)</h2>
            <div style={styles.selectionGrid}>
              {allFruits.map((f) => (<button key={f} onClick={() => setFruits(fruits.includes(f)
                    ? fruits.filter((x) => x !== f)
                    : [...fruits, f])} className={`selection-card${fruits.includes(f) ? ' selected' : ''}`}>
                  {f}
                </button>))}
            </div>
            <div className="custom-input-row">
              <input type="text" placeholder="Add other fruit..." value={customFruitInput} onChange={(e) => setCustomFruitInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    addCustomItem(customFruitInput, allFruits, setCustomFruits, setCustomFruitInput);
                }
            }}/>
              <button onClick={() => addCustomItem(customFruitInput, allFruits, setCustomFruits, setCustomFruitInput)}>
                Add
              </button>
            </div>
            {fruits.length > 0 && (<p style={styles.selectionSummary}>Selected: {fruits.join(', ')}</p>)}
        ...
        {step === 3 && (<div style={styles.stepContent}>
            <h2 style={styles.stepTitle}>Cuisine Style</h2>
            <div style={styles.selectionGrid}>
              {allCuisines.map((c) => (<button key={c} onClick={() => setCuisine(cuisine === c ? null : c)} className={`selection-card${cuisine === c ? ' selected' : ''}`}>
                  {c}
                </button>))}
            </div>
            <div className="custom-input-row">
              <input type="text" placeholder="Add other cuisine..." value={customCuisineInput} onChange={(e) => setCustomCuisineInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    addCustomItem(customCuisineInput, allCuisines, setCustomCuisines, setCustomCuisineInput);
                }
            }}/>
              <button onClick={() => addCustomItem(customCuisineInput, allCuisines, setCustomCuisines, setCustomCuisineInput)}>
                Add
              </button>
            </div>
            ...
            <div style={styles.selectionGrid}>
              {allMethods.map((m) => (<button key={m} onClick={() => setCookingMethods(cookingMethods.includes(m)
                    ? cookingMethods.filter((x) => x !== m)
                    : [...cookingMethods, m])} className={`selection-card${cookingMethods.includes(m) ? ' selected' : ''}`}>
                  {m}
                </button>))}
            </div>
            <div className="custom-input-row">
              <input type="text" placeholder="Add other method..." value={customMethodInput} onChange={(e) => setCustomMethodInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    addCustomItem(customMethodInput, allMethods, setCustomMethods, setCustomMethodInput);
                }
            }}/>
              <button onClick={() => addCustomItem(customMethodInput, allMethods, setCustomMethods, setCustomMethodInput)}>
                Add
              </button>
            </div>
            {cookingMethods.length > 0 && (<p style={styles.selectionSummary}>Selected: {cookingMethods.join(', ')}</p>)}
        ...
        {step === 4 && (<div style={styles.stepContent}>
            <h2 style={styles.stepTitle}>How much time do you have?</h2>
            <div style={styles.timeGrid}>
              {TIME_OPTIONS.map((t) => (<button key={t.value} onClick={() => setTimeConstraint(timeConstraint === t.value ? null : t.value)} className={`selection-card${timeConstraint === t.value ? ' selected' : ''}`} style={{ textAlign: 'center' }}>
            ...
            <div style={styles.servingsControl}>
              <button onClick={() => setServings(Math.max(1, servings - 1))} className="btn-secondary" style={{ width: '40px', height: '40px', borderRadius: '50%', padding: 0, fontSize: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                −
              </button>
              <span style={styles.servingsValue}>{servings}</span>
              <button onClick={() => setServings(Math.min(12, servings + 1))} className="btn-secondary" style={{ width: '40px', height: '40px', borderRadius: '50%', padding: 0, fontSize: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                +
              </button>
            </div>
            ...
            <div style={styles.selectionGrid}>
              {RECIPE_TYPE_OPTIONS.map((t) => (<button key={t.value} onClick={() => setRecipeType(t.value)} className={`selection-card${recipeType === t.value ? ' selected' : ''}`}>
                  {t.label}
                </button>))}
            </div>
        ...
        {step === 5 && (<div style={styles.stepContent}>
            {generating ? (<div style={styles.loadingContainer}>
              ...
              </div>) : recipe ? (<div style={styles.previewContainer}>
                ...
                <ul style={styles.ingredientList}>
                  {recipe.ingredients.slice(0, 5).map((ing, i) => (<li key={i} style={styles.ingredientItem}>
                      {ing.quantity} {ing.unit} {ing.name}
                      {ing.preparation && `, ${ing.preparation}`}
                    </li>))}
                  {recipe.ingredients.length > 5 && (<li style={styles.moreItems}>
                ...
                <ol style={styles.instructionList}>
                  {recipe.instructions.slice(0, 2).map((step) => (<li key={step.stepNumber} style={styles.instructionItem}>
                      {step.text}
                    </li>))}
                  {recipe.instructions.length > 2 && (<li style={styles.moreItems}>
