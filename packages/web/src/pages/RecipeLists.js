...
export default function RecipeLists() {
    ...
    if (isPending || loading) {
        ...
        <div style={styles.listGrid}>
          {Array.from({ length: 3 }).map((_, i) => (<div key={i} className="skeleton-card" style={{ padding: '1rem 1.25rem' }}>
    ...
    return (<div style={styles.container}>
      <div style={styles.header}>
        ...
        <button onClick={() => setShowCreate(!showCreate)} className="btn-primary" style={{ fontSize: '0.875rem', padding: '0.75rem 1.25rem' }}>
          + New List
        </button>
      </div>
      ...
      {showCreate && (<div style={styles.createForm}>
          <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="List name" style={styles.input} autoFocus onKeyDown={(e) => e.key === 'Enter' && handleCreate()}/>
          <input type="text" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Description (optional)" style={styles.input} onKeyDown={(e) => e.key === 'Enter' && handleCreate()}/>
          <div style={styles.createActions}>
            <button onClick={handleCreate} disabled={creating || !newName.trim()} className="btn-primary" style={{ fontSize: '0.875rem', padding: '0.625rem 1.25rem' }}>
            ...
            <button onClick={() => {
                ...
                setNewName('');
                setNewDescription('');
            }} className="btn-secondary" style={{ fontSize: '0.875rem', padding: '0.625rem 1.25rem' }}>
              Cancel
            </button>
          </div>
        </div>)}

      {lists.length === 0 && !showCreate ? (<div style={styles.emptyState}>
          ...
          {lists.map((list) => (<Link key={list.id} to={`/recipe-lists/${list.id}`} className="list-card">
              ...
              {list.description && (<p style={styles.listDescription}>{list.description}</p>)}
              {list.recipes.length > 0 && (<p style={styles.listPreview}>
                  {list.recipes
                        .slice(0, 3)
                        .map((r) => r.title)
                        .join(', ')}
