...
function StarDisplay({ rating, count }) {
    ...
    return (<span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8125rem' }}>
      {stars}
...
export default function Discover() {
    ...
    return (<div style={styles.container}>
      ...
      <div style={styles.searchBar}>
        <input type="text" placeholder="Search recipes..." value={search} onChange={(e) => handleSearchChange(e.target.value)} className="edit-input" style={styles.searchInput}/>
      </div>

      {error && <div className="error-message" style={{ maxWidth: '900px', margin: '0 auto 1rem' }}>{error}</div>}

      {loading ? (<div style={styles.recipeGrid}>
        ...
        </div>) : recipes.length === 0 ? (<div style={styles.emptyState}>
          ...
          <div style={styles.recipeGrid}>
            {recipes.map((recipe) => (<Link key={recipe.id} to={`/discover/recipes/${recipe.id}`} className="recipe-card">
                ...
                {recipe.checkinCount > 0 && (<div style={{ marginBottom: '0.75rem' }}>
          ...
          {pagination && pagination.totalPages > 1 && (<div style={styles.pagination}>
              <button onClick={() => handlePageChange(pagination.page - 1)} disabled={pagination.page <= 1} className="btn-secondary" style={styles.pageBtn}>
                Previous
              </button>
              <span style={styles.pageInfo}>
                Page {pagination.page} of {pagination.totalPages}
              </span>
              <button onClick={() => handlePageChange(pagination.page + 1)} disabled={pagination.page >= pagination.totalPages} className="btn-secondary" style={styles.pageBtn}>
                Next
              </button>
            </div>)}
        </>)}
