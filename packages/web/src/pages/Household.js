...
export default function HouseholdPage() {
    ...
    const handleRegenerateCode = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                setHousehold((prev) => prev ? { ...prev, inviteCode: data.data.inviteCode } : null);
    ...
    if (isPending || loading) {
        ...
        <div style={styles.card}>
          ...
          <div>
            ...
            {Array.from({ length: 2 }).map((_, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 0', borderBottom: `1px solid ${colors.border}` }}>
    ...
    return (<div style={styles.container}>
      <div style={styles.card}>
        ...
        <div>
            ...
            <div style={styles.section}>
              ...
              <ul style={styles.memberList}>
                {household.members.map((member) => (<li key={member.id} style={styles.memberItem}>
        ...
        <div>
            ...
            {!showCreateForm && !showJoinForm && (<div style={styles.options}>
                <button onClick={() => setShowCreateForm(true)} className="btn-primary" style={{ padding: '1rem', fontSize: '1rem', width: '100%' }}>
                  Create Household
                </button>
                <button onClick={() => setShowJoinForm(true)} className="btn-secondary" style={{ padding: '1rem', fontSize: '1rem', width: '100%' }}>
                  Join with Code
                </button>
              </div>)}

            {showCreateForm && (<form onSubmit={handleCreate} style={styles.form}>
                <h3 style={styles.formTitle}>Create Household</h3>
                <div style={styles.field}>
                  <label style={styles.label}>Household Name</label>
                  <input type="text" value={householdName} onChange={(e) => setHouseholdName(e.target.value)} placeholder="e.g., Smith Family" style={styles.input} required/>
                </div>
                <div style={styles.formActions}>
                  <button type="button" onClick={() => setShowCreateForm(false)} className="btn-secondary" style={{ padding: '0.5rem 1rem' }}>
                    Cancel
                  </button>
            ...
            {showJoinForm && (<form onSubmit={handleJoin} style={styles.form}>
                <h3 style={styles.formTitle}>Join Household</h3>
                <div style={styles.field}>
                  <label style={styles.label}>Invite Code</label>
                  <input type="text" value={inviteCode} onChange={(e) => setInviteCode(e.target.value.toUpperCase())} placeholder="e.g., ABC12345" style={{ ...styles.input, textTransform: 'uppercase', letterSpacing: '2px' }} required/>
                </div>
                <div style={styles.formActions}>
                  <button type="button" onClick={() => setShowJoinForm(false)} className="btn-secondary" style={{ padding: '0.5rem 1rem' }}>
                    Cancel
                  </button>
                  <button type="submit" className="btn-primary" style={{ padding: '0.5rem 1rem' }} disabled={submitting}>
