...
import { db } from '../db';
import { groceryLists, groceryListItems, recipes, recipeIngredients, ingredients, } from '../db/schema';
...
function convertFromBaseUnit(quantity, baseUnit) {
...
function roundQuantity(qty) {
    ...
    if (qty >= 1)
        return Math.round(qty * 4) / 4; // Quarter precision
...
groceryListsRouter.post('/', async (c) => {
    ...
    const { name, recipes: recipeSelections } = body;
    ...
    const selectedRecipes = await db
        ...
        .from(recipes)
        .where(and(eq(recipes.householdId, currentUser.householdId), inArray(recipes.id, recipeIds)));
    ...
    for (const agg of aggregated.values()) {
        const { quantity: finalQty, unit: finalUnit } = convertFromBaseUnit(agg.baseQuantity, agg.baseUnit);
...
groceryListsRouter.get('/:id', async (c) => {
    ...
    const [groceryList] = await db
        .select()
        .from(groceryLists)
        .where(and(eq(groceryLists.id, id), eq(groceryLists.householdId, currentUser.householdId)))
        .limit(1);
    if (!groceryList) {
...
groceryListsRouter.patch('/:id/items/:itemId', async (c) => {
    ...
    const [groceryList] = await db
        .select()
        .from(groceryLists)
        .where(and(eq(groceryLists.id, listId), eq(groceryLists.householdId, currentUser.householdId)))
        .limit(1);
    if (!groceryList) {
        return c.json({ error: 'Grocery list not found' }, 404);
    }
