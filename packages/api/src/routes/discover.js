...
discoverRouter.get('/recipes', async (c) => {
    ...
    if (search) {
        conditions.push(or(ilike(recipes.title, `%${search}%`), ilike(recipes.description, `%${search}%`)));
    }
    if (cuisine) {
    ...
    const results = await db
        ...
        avgEnjoymentRating: sql `coalesce(avg(${recipeCheckins.enjoymentRating}), 0)`.as('avg_enjoyment_rating'),
        avgInstructionRating: sql `coalesce(avg(${recipeCheckins.instructionRating}), 0)`.as('avg_instruction_rating'),
        checkinCount: sql `count(${recipeCheckins.id})`.as('checkin_count'),
    })
        .from(recipes)
        ...
        .offset(offset);
    // Get creator names
    const creatorIds = [...new Set(results.map((r) => r.createdByUserId).filter(Boolean))];
    const creators = {};
...
discoverRouter.get('/recipes/:id', async (c) => {
    ...
    const [ratings] = await db
        .select({
        avgEnjoymentRating: sql `coalesce(avg(${recipeCheckins.enjoymentRating}), 0)`,
        avgInstructionRating: sql `coalesce(avg(${recipeCheckins.instructionRating}), 0)`,
        checkinCount: sql `count(${recipeCheckins.id})`,
    })
        .from(recipeCheckins)
...
discoverRouter.post('/recipes/:id/copy', async (c) => {
    ...
    if (sourceRecipe.recipeIngredients.length > 0) {
        await db.insert(recipeIngredients).values(sourceRecipe.recipeIngredients.map((ri) => ({
